...
class LoaderExtensionOutputGenerator(OutputGenerator):
    ...
    def AddManualTrampTermFuncs(self):
        ...
        funcs += '        if ((VK_SUCCESS != res && VK_INCOMPLETE != res) || NULL == pPhysicalDeviceGroupProperties) {\n'
        funcs += '            goto out;\n'
        funcs += '        }\n'
        funcs += '    }\n'
        funcs += '\n'
        funcs += '    if (NULL == pPhysicalDeviceGroupProperties) {\n'
        funcs += '        // Goto out, even on success since we don\'t need to fill in the rest.\n'
        funcs += '        goto out;\n'
        funcs += '    }\n'
        funcs += '\n'
        ...
        funcs += '\n'
        funcs += '    if (pPhysicalDeviceGroupProperties == NULL || 0 == inst->total_gpu_count) {\n'
        funcs += '        res = setupLoaderTermPhysDevs(inst);\n'
        funcs += '        if (VK_SUCCESS != res) {\n'
        funcs += '            goto out;\n'
        funcs += '        }\n'
        funcs += '    }\n'
        funcs += '\n'
        funcs += '	if (pPhysicalDeviceGroupProperties != NULL) {\n'
        funcs += '		memset(pPhysicalDeviceGroupProperties, 0, sizeof(VkPhysicalDeviceGroupPropertiesKHX) * max_group_count);\n'
        funcs += '	}\n'
        funcs += '\n'
        ...
        funcs += '                if (res != VK_SUCCESS && res != VK_INCOMPLETE) {\n'
        funcs += '                    goto out;\n'
        funcs += '                }\n'
        funcs += '\n'
        funcs += '                for (uint32_t j = 0; j < remain_count; j++) {\n'
        funcs += '                    i += pPhysicalDeviceGroupProperties[total_group_count + j].physicalDeviceCount;\n'
        funcs += '                }\n'
        funcs += '                total_group_count += remain_count;\n'
        funcs += '            } else {\n'
        funcs += '                total_group_count += cur_group_count;\n'
        funcs += '            }\n'
        funcs += '        } else {\n'
        funcs += '            if (NULL != pPhysicalDeviceGroupProperties && max_group_count > total_group_count) {\n'
        funcs += '                pPhysicalDeviceGroupProperties[total_group_count].sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_GROUP_PROPERTIES_KHX;\n'
        funcs += '                pPhysicalDeviceGroupProperties[total_group_count].pNext = NULL;\n'
        ...
        funcs += '                    inst->phys_devs_term[i]->phys_dev;\n'
        funcs += '                pPhysicalDeviceGroupProperties[total_group_count].subsetAllocation = VK_FALSE;\n'
        funcs += '            }\n'
        ...
        funcs += '    // so we can dereference them if needed.\n'
        funcs += '    if (NULL != pPhysicalDeviceGroupProperties) {\n'
        ...
        funcs += '                for (uint32_t gpu = 0; gpu < inst->total_gpu_count; gpu++) {\n'
        funcs += '                    if (inst->phys_devs_term[gpu]->phys_dev == cur_props->physicalDevices[i]) {\n'
        funcs += '                        cur_props->physicalDevices[i] = (VkPhysicalDevice)inst->phys_devs_term[gpu];\n'
        funcs += '                        break;\n'
        funcs += '                    }\n'
        funcs += '                }\n'
        funcs += '            }\n'
